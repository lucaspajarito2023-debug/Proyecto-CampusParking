...
function showProfile(){
    ...
    setTimeout(() => {dynamicContent.innerHTML = `
...
function saveProfile(){
    ...
    const updatedAdmin = {name:document.getElementById("newName").value,
...
function showEntryForm(){
    ...
    <input type="text" id="plate" placeholder="P123ABC">
    <button onclick="assignSlot()">Registrar</button>
    `;
}
...
function calculateEarnings(plate){
    ...
    const durationMinutes = Math.ceil(( exitTime - vehicle.entryTime) / (1000 * 60));
    let rate = 0;
    switch(vehicle.type){
        case "Moto":
            rate = 0.5;
...
function finishService(plate){
    ...
    setTimeout(() => {
        const exit =new Date();
        const total =calculateEarnings(plate);
        // SUMAR GANANCIAS
        totalEarnings += total;
...
//********* Resetear Ganancias cada 24 horas *********
setInterval(() => {
    totalEarnings = 0;
    localStorage.setItem("totalEarnings", JSON.stringify(totalEarnings));
    updateSidebar();
